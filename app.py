...
if st.session_state.perfil == "Admin":
    ...
    with menu[0]:
        if df_clientes.empty:
            ...
            busca = st.text_input("Digite o Nome, Placa ou CPF do cliente:", value=st.session_state.busca_input)
            ...
            if not df_filtrado_cli.empty:
                ...
                motivo_servico = st.selectbox("Motivo:", ["Acidente", "Furto", "Roubo", "Outros"])
                prestador_manual = st.text_input("Prestador (Nome e Telefone):")
                
                localizacao = st.text_input("Origem:")
                ...
                if st.button("🚀 Iniciar Atendimento / Gerar OS"):
                    ...
                    nova_os = pd.DataFrame([{
                        ...
                        'tipo_servico': tipo_servico, 'motivo': motivo_servico, 'prestador': prestador_manual,
                ...
                if "ultima_os_id" in st.session_state:
                    ...
                    with col_zap:
                        link_w = f"https://api.whatsapp.com/send?phone=55{apenas_numeros_letras(prestador_manual)}&text=OS%20{os_id}"
                        st.markdown(f'<a href="{link_w}" target="_blank"><button style="background-color: #25D366; color: white; padding: 10px 20px; border: none; border-radius: 4px; font-weight: bold; cursor: pointer;">➡️ WhatsApp Prestador</button></a>', unsafe_allow_html=True)
                    with col_close:
    ...
    with menu[1]:
        ...
        for idx, row in df_os.sort_values(by='id', ascending=False).iterrows():
            st.write(f"OS {row['id']} | {row['cliente_nome']} | {row['status_os']}")
    ...
    with menu[4]:
        ...
        # A MELHORIA SOLICITADA: Multiselect
        ...
        if st.button("Salvar Prestador"):
            ...
            if modo_p and dados_p is not None:
                ...
                novo_p = pd.DataFrame([{'id': str(int(df_prestadores['id'].astype(float).max()+1) if not df_prestadores.empty else 1), 'nome': n_p.upper(), 'tipo': tipo_f, 'telefone': '', 'est': 'RN', 'status': 'Ativo'}])
