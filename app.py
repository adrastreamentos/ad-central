...
if st.session_state.perfil == "Admin":
    ...
    with menu[4]:
        ...
        if opcao_p == "Listar":
            ...
            # Campo de serviço alterado para permitir seleção múltipla com Guincho como padrão
            t_prest_sel = st.multiselect("Tipo de Serviço prestado:", SERVICOS_DISPONIVEIS, default=["Guincho"], key="p_tipo")
            ...
            if st.button("Salvar Prestador", key="save_prest_btn_novo"):
                n_prest = n_prest_in.upper() if n_prest_in else (dados_p_ant['nome'] if dados_p_ant is not None else "")
                t_prest = ", ".join(t_prest_sel) if t_prest_sel else "Guincho"
                tel_p = apenas_numeros_letras(tel_p_raw) if tel_p_raw else (dados_p_ant['telefone'] if dados_p_ant is not None else "")
