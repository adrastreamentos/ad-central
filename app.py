...
def gerar_botao_whatsapp(dados_dict):
    ...
    st.markdown(f'<a href="{link}" target="_blank" style="text-decoration: none;"><button style="background-color: #25D366; color: white; padding: 12px 24px; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; width: 100%; margin-top: 10px;">📲 Informar Falha e Enviar Dados via WhatsApp</button></a>', unsafe_allow_html=True)
...
if not st.session_state.logado:
    # Lendo o parâmetro da URL: ?portal=prestador
    ...
    with col_meio:
        if portal == "prestador":
            ...
            with tab_cadastro:
                st.info("Preencha o formulário para enviar sua solicitação de parceria para a Central AD.")
                with st.form("form_cad_prestador"):
                    ...
                    cad_senha = st.text_input("Crie uma Senha para acessar o aplicativo depois de aprovado:", type="password")
                    ...
                    if st.form_submit_button("Enviar Solicitação de Cadastro", use_container_width=True):
                        if not cad_nome or not cad_cpf or not cad_senha:
                            ...
                            with st.spinner("Enviando para a Central AD..."):
                                ...
                                if sucesso:
                                    st.success("✅ Cadastro enviado! Aguarde a aprovação da Central.")
                                    time.sleep(2); st.rerun()
                                else:
                                    st.error("Erro ao salvar cadastro. Tente novamente.")
        
        else:
            # Login Central e Empresas (Link Normal)
    ...
    with menu[7]:
        ...
        if empresas_ativas.empty:
            ...
            for _, r in df_view_fin.iterrows():
                try: 
                    vf = float(str(r['valor_faturado']).replace(',','.'))
                    vp = float(str(r['valor_pago']).replace(',','.'))
            ...
            if emp_edit:
                ...
                with st.form("form_financeiro"):
                    ...
                    try:
                        ...
                        if modo_fat_edit == 'Performance (Escalonado)' and v2 < v1 and status_final == 'Pago':
                            st.warning("⚠️ Atenção: O valor pago digitado é MENOR que a fatura calculada. A diferença ficará registrada na tabela.")
                    except: pass
