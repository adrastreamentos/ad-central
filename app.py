...
def exportar_pdf_html_oficial(df_os, df_clientes, nome_arquivo):
    ...
    r = df_os.iloc[0]
    fotos_html = ""
    vistoria_path = os.path.join("AD_Assistencia", "vistorias", str(r['id']))
    if os.path.exists(vistoria_path):
        fotos_html += "<h3 style='color: #7B2CBF; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 5px;'>📸 Vistoria e Assinatura</h3><div style='display: flex; flex-wrap: wrap; gap: 15px; margin-top: 10px;'>"
        for foto_nome in ['Frente', 'Traseira', 'Lateral_Esquerda', 'Lateral_Direita', 'Placa', 'Assinatura']:
            foto_file = os.path.join(vistoria_path, f"{foto_nome}.jpg")
            if os.path.exists(foto_file):
                try:
                    with open(foto_file, "rb") as img_file:
                        b64_img = base64.b64encode(img_file.read()).decode('utf-8')
                        titulo_foto = foto_nome.replace('_', ' ')
                        fotos_html += f"<div style='border: 1px solid #ddd; padding: 5px; border-radius: 5px; background: #fff;'><p style='text-align: center; margin: 2px 0 5px 0; font-size: 12px; font-weight: bold; color: #555;'>{titulo_foto}</p><img src='data:image/jpeg;base64,{b64_img}' style='width: 200px; height: auto; border-radius: 3px;'></div>"
                except: pass
        fotos_html += "</div>"
    html = f"""
    <html>
    ...
    <html>
        ...
        <body>
            ...
            <div class="kpi-container">
                <div class="kpi-card"><h4>Empresa / Escopo</h4><p>{empresa_filtro.upper()}</p></div>
                <div class="kpi-card"><h4>Frota Ativa Mapeada</h4><p>{total_veiculos} veículos</p></div>
                <div class="kpi-card"><h4>Total de Ocorrências</h4><p>{total_os} chamados</p></div>
            </div>
            <h3 style="color: #4a148c; font-size: 16px;">📊 Distribuição Percentual de Eventos</h3>
            <table>
                <thead><tr><th>Tipo de Ocorrência</th><th style="text-align: center;">Quantidade de Acionamentos</th><th style="text-align: right;">Participação (%)</th></tr></thead>
...
def renderizar_dashboard(empresa_filtro="Todas"):
    ...
    if total_os > 0:
        ...
        with col_chart1:
            ...
            fig_incidencia = px.pie(servicos_counts, names='Serviço', values='Quantidade', hole=0.55, color_discrete_sequence=['#7B2CBF', '#E53935', '#1976D2', '#2E7D32', '#FF9800', '#9C27B0'])
            fig_incidencia.update_traces(textposition='inside', textinfo='percent')
            fig_incidencia.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=280, legend=dict(orientation="v", y=0.5, x=1.0))
        ...
        with col_chart2:
            ...
            fig_status = px.pie(status_counts, names='Status', values='Quantidade', hole=0.55, color_discrete_sequence=['#2E7D32', '#E53935', '#FF9800', '#1976D2'])
            fig_status.update_traces(textposition='inside', textinfo='percent')
            fig_status.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=280, legend=dict(orientation="v", y=0.5, x=1.0))
        ...
        if not st.session_state.exibir_laudo:
            ...
            st.markdown("""<div style="background-color: #f8f9fa; border: 2px solid #7B2CBF; border-radius: 12px; padding: 20px; margin-bottom: 20px;"><h3 style="color: #7B2CBF; margin-top:0;">📋 Laudo Executivo de Atendimentos 24h</h3><p style="color: #555; font-size: 14px; margin-bottom: 0;">Análise consolidada e inteligência de ocorrências da frota no período.</p></div>""", unsafe_allow_html=True)
            ...
            with col_laudo_btn2:
                if st.button("❌ Fechar Laudo Executivo", use_container_width=True, key=f"btn_close_laudo_{empresa_filtro}"):
                    ...
                    st.rerun()
    else: st.info("Nenhuma ordem de serviço registrada para montar os indicadores do período.")


# ===================================================================================
# FUNÇÕES DE CÁLCULO E REGRAS DE NEGÓCIO (FINANCEIRO)
# ===================================================================================
def colorir_status(val): return 'color: #2e7d32; font-weight: bold;' if str(val).strip() == 'Ativo' else 'color: #c62828; font-weight: bold;'
...
def calcular_fatura_parceiro(nome_empresa, mes, ano, df_clientes_atuais, df_os_atuais, df_empresas_atuais):
    ...
    for _, r_cli in df_cli_fat.iterrows():
        ...
        if not placas_extraidas:
            ...
            p2 = str(r_cli.get('pla_2', '')).strip().upper().replace("-","").replace(" ","")
            if len(p2) >= 6 and p2 not in ['NAN', 'N/D']: placas_extraidas.append(p2)
        for placa in placas_extraidas: lista_veiculos_emp.append({'placa': placa, 'plano': plano_limpo, 'cliente': str(r_cli.get('nome', '')).upper()})
    ...
    os_filtro = df_os_temp[(df_os_temp['empresa'].str.upper() == nome_empresa.upper()) & (df_os_temp['status_os'].str.upper() == 'ENCERRADO') & (df_os_temp['data_hora'] >= dt_inicio) & (df_os_temp['data_hora'] <= dt_fim)]
    ...
    if modo_fat_calc == "Performance (Escalonado)":
        ...
        if "Frota Pequena" in modo_fat_calc: valor_base, acionamentos_isentos, taxa_50, taxa_100 = 300.00, 2, 50.00, 85.00
        elif "Opção A" in modo_fat_calc: valor_base, acionamentos_isentos, taxa_50, taxa_100 = 500.00, 2, 50.00, 85.00
        elif "Opção B" in modo_fat_calc: valor_base, acionamentos_isentos, taxa_50, taxa_100 = 500.00, 4, 80.00, 130.00
...
def gerar_texto_resumo_plano(dados_fat):
    ...
    if modo == "Performance (Escalonado)":
        ...
        texto += f"📈 <b>Enquadramento Comercial:</b> {faixa}<br>💰 <b>Fatura Mapeada:</b> R$ {fat_tot:.2f}"
        return texto
    elif "Frota Pequena" in modo or "Até 40" in modo:
        tot_ac = dados_fat.get('total_ac', 0)
        isentos = dados_fat.get('acionamentos_isentos', 0)
        texto = f"{periodo_str}<br>📊 <b>Plano Ativo:</b> {modo}<br>🚗 <b>Base Apurada:</b> {tot_v} veículos<br>🎟️ <b>Franquia Utilizada:</b> {tot_ac} de {isentos} guinchos<br>"
        if exc > 0:
            detalhe_exc = []
        ...
        else: texto += f"💰 <b>Fatura Atual: R$ {fat_tot:.2f}</b> <span style='color:#2e7d32; font-size:14px;'>(Uso operando perfeitamente dentro da franquia gratuita do pacote)</span>"
        return texto
    else:
...
def gerar_pdf_extrato_detalhado(nome_empresa, mes, ano, df_clientes_atuais, df_os_atuais, df_empresas_atuais):
    ...
    if os_mes.empty: linhas_os_html = "<tr><td colspan='6' style='text-align: center; padding: 10px; color: #666;'>Nenhum acionamento registrado neste ciclo.</td></tr>"
        ...
        for _, r in os_mes.iterrows(): linhas_os_html += f"<tr><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px;'>{r['id']}</td><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px;'>{str(r['data_hora'])[:16]}</td><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px; font-weight: bold;'>{r['placa']}</td><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px;'>{r['cliente_nome']}</td><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px;'>{r['tipo_servico']}</td><td style='border: 1px solid #ddd; padding: 8px; font-size: 12px;'>{r['localizacao']} ➔ {r['destino']}</td></tr>"
    linhas_veiculos_html = ""
    for idx_v, v in enumerate(dados_fat['veiculos']): linhas_veiculos_html += f"<tr><td style='border: 1px solid #ddd; padding: 6px; font-size: 11px;'>{idx_v + 1}</td><td style='border: 1px solid #ddd; padding: 6px; font-size: 11px; font-weight: bold;'>{v['placa']}</td><td style='border: 1px solid #ddd; padding: 6px; font-size: 11px;'>{v['cliente']}</td><td style='border: 1px solid #ddd; padding: 6px; font-size: 11px;'>{v['plano']}</td><td style='border: 1px solid #ddd; padding: 6px; font-size: 11px;'>{v['tipo_cobranca']}</td></tr>"
    str_inicio, str_fim = dados_fat['dt_inicio'].strftime('%d/%m/%Y'), dados_fat['dt_fim'].strftime('%d/%m/%Y')
...
# ===================================================================================
# PORTAL DO CLIENTE (NPS E CAPTURA DE GPS)
# ===================================================================================
...
if portal_atual == "nps":
    ...
    if not os_param: st.error("Número de atendimento não identificado."); st.stop()
    os_relacionada = df_os[df_os['id'].astype(str) == str(os_param)]
    if os_relacionada.empty: st.error("Ordem de serviço não encontrada no sistema."); st.stop()
    os_info = os_relacionada.iloc[0]
    nps_existente = df_nps[df_nps['id_os'].astype(str) == str(os_param)]
    if not nps_existente.empty: st.info("✅ Esta ordem de serviço já foi avaliada. Muito obrigado pelo seu tempo!"); st.stop()

    st.write(f"**Veículo:** {os_info['placa']}"); st.write(f"**Data do Chamado:** {os_info['data_hora'][:10]}"); st.write("---")
    ...
    if st.button("🚀 Enviar minha avaliação", use_container_width=True, type="primary"):
        ...
        novo_nps = pd.DataFrame([{'id_os': os_param, 'data_hora': obter_hora_str(), 'nota_nps': nota_nps, 'nota_central': val_cen, 'nota_guincho': val_gui, 'comentario': comentario.strip(), 'status_nps': status_calc}])
...
if portal_atual == "cliente":
    ...
    html_code = f"""
    <!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>body {{ font-family: Arial, sans-serif; text-align: center; padding: 20px; background-color: #f8f9fa; margin: 0; }}.btn {{ background-color: #E53935; color: white; padding: 20px; font-size: 18px; border: none; border-radius: 8px; cursor: pointer; width: 100%; max-width: 300px; font-weight: bold; box-shadow: 0 4px 6px rgba(0,0,0,0.2); margin-top: 20px; }}.btn:active {{ background-color: #b71c1c; }}#msg {{ margin-top: 20px; font-size: 16px; color: #555; font-weight: bold; }}</style></head><body><h3 style="color: #7B2CBF; margin-top:0;">Localização de Emergência</h3><p>Precisamos saber onde você está para enviar o guincho exato até o veículo placa <b>{placa_param}</b>.</p><button id="btn-loc" class="btn" onclick="getLocation()">📍 ENVIAR MINHA LOCALIZAÇÃO</button><p id="msg"></p><script>function getLocation() {{ document.getElementById("msg").innerHTML = "Aguardando GPS... Autorize se o celular pedir."; document.getElementById("btn-loc").style.display = "none"; if (navigator.geolocation) {{ navigator.geolocation.getCurrentPosition(showPosition, showError, {{enableHighAccuracy: true}}); }} else {{ document.getElementById("msg").innerHTML = "Seu navegador não suporta GPS."; document.getElementById("btn-loc").style.display = "block"; }} }} function showPosition(position) {{ var lat = position.coords.latitude; var lon = position.coords.longitude; document.getElementById("msg").innerHTML = "Sinal capturado! Salvando no sistema... 🚀"; var urlFinal = "https://ad-central-mrssupqbb9ux69bi4qgisa.streamlit.app/?portal=cliente_salvo&placa={placa_param}&lat=" + lat + "&lon=" + lon; try {{ window.top.location.href = urlFinal; }} catch(e) {{ console.log("Fallback acionado."); }} setTimeout(function() {{ window.location.href = urlFinal; }}, 500); }} function showError(error) {{ document.getElementById("btn-loc").style.display = "block"; switch(error.code) {{ case error.PERMISSION_DENIED: document.getElementById("msg").innerHTML = "❌ Você negou o acesso ao GPS. Libere a permissão e tente novamente."; break; case error.POSITION_UNAVAILABLE: document.getElementById("msg").innerHTML = "❌ Sinal de GPS indisponível no momento."; break; case error.TIMEOUT: document.getElementById("msg").innerHTML = "❌ Tempo esgotado para buscar o GPS."; break; default: document.getElementById("msg").innerHTML = "❌ Erro desconhecido ao tentar localizar."; break; }} }}</script></body></html>
    """
    ...
    lat, lon = st.query_params.get("lat", ""), st.query_params.get("lon", "")
    if lat and lon:
    ...
    else: st.error("Erro ao receber as coordenadas. Tente novamente.")
    st.stop()
...
if not st.session_state.logado:
    st.markdown('<div class="main-title">AD Rastreamento Veicular <span style="font-size: 16px; color: #ccc;">🚀 v9.8</span></div>', unsafe_allow_html=True)
            ...
            with tab_login:
                ...
                if st.button("Acessar Meu Painel", use_container_width=True, key="btn_login_prest"):
                    ...
                    if not prestador_valid.empty:
                        ...
                        else: st.session_state.update({"logado": True, "user": prest_row['nome'].upper(), "perfil": "Prestador", "empresa_vinculada": ""}); time.sleep(0.5); st.rerun()
...
with col_user: st.markdown(f"<div style='font-size: 18px; font-weight: bold; color: #4a148c; padding-top: 5px;'>Central AD 24h | Operador: <span style='color: #E53935;'>{st.session_state.user}</span></div>", unsafe_allow_html=True)
with col_refresh:
    if st.button("🔄 Atualizar Sistema", use_container_width=True): st.rerun()
with col_logout:
    if st.button("Sair / Logoff", key="btn_logout_master", use_container_width=True): st.session_state.logado = False; st.query_params.clear(); st.rerun()
st.write("---")
...
if st.session_state.perfil == "Admin":
    ...
    if aba_selecionada != aba_atual_admin: st.query_params["nav"] = aba_selecionada; st.rerun()
    ...
    if aba_selecionada == "📈 Dashboard":
        ...
        pronto_para_prosseguir, is_excecao_flag = False, False
        valor_excecao_val, valor_cobrado_os = "0,00", "0,00"
        cliente_id_os, cliente_nome_os, placa_alvo, veiculo_desc_alvo, empresa_os, plano_km_os, uf_cliente, cidade_cliente = "", "", "", "", "", "", "RN", ""
        tel_envio_link = ""
                    ...
                    if df_filtrado_cli.empty: st.error("Nenhum cliente ou veículo encontrado com esse termo de busca.")
                        ...
                        if c_target_os != "":
                            ...
                            if not lista_frota_opcoes: st.error("Este cliente não possui veículos cadastrados com placa válida.")
                                ...
                                os_placa_ano = df_os_carencia[(df_os_carencia['placa'].astype(str).apply(lambda x: apenas_numeros_letras(x).upper()) == placa_alvo_limpa) & (~df_os_carencia['status_os'].str.upper().isin(['CANCELADO'])) & (df_os_carencia['data_hora'] >= inicio_ciclo) & (df_os_carencia['data_hora'] <= fim_ciclo)]
                                ...
                                limite_excedido = uso_atual.get(tipo_servico.upper(), 0) >= LIMITES_ANUAIS.get(tipo_servico.upper(), 1)
                                os_recentes = df_os_carencia[(df_os_carencia['placa'].astype(str).apply(lambda x: apenas_numeros_letras(x).upper()) == placa_alvo_limpa) & (~df_os_carencia['status_os'].str.upper().isin(['CANCELADO']))]
                                
                                bloqueio_60, msg_bloqueio_60 = False, ""
                                if not os_recentes.empty:
                                    ...
                                    if 0 <= dias_passados < 60:
                                        bloqueio_60, data_ult = True, ultima_os['data_hora'].strftime("%d/%m/%Y")
                                ...
                                cliente_inativo = (str(cliente_dados.get('status', 'Ativo')).strip() == 'Inativo')
                                if cliente_inativo or bloqueio_60 or limite_excedido:
                                    ...
                                    if st.checkbox("⚠️ Ciente dos bloqueios acima: Liberar Atendimento por Exceção"): 
                                        valor_excecao_val = st.text_input("Valor do Serviço Extra a ser Cobrado (R$):", value="0,00")
                                        valor_cobrado_os, is_excecao_flag, pronto_para_prosseguir = valor_excecao_val, True, True
        ...
        if pronto_para_prosseguir:
            ...
            if not df_prestadores.empty:
                ...
                if not df_prest_filtrados.empty:
                    ...
                    for _, r in df_prest_filtrados.iterrows(): lista_p_ops.append(f"{'📍 [MAIS PRÓXIMO] ' if r['prioridade'] == 0 else ''}{str(r['nome'])} - Tel: {str(r['telefone'])} - {str(r['cidade']).upper()}/{str(r['est']).upper()}")
                else:
                    for _, r in df_prestadores[df_prestadores['homologado'] == 'Aprovado'].iterrows(): lista_p_ops.append(f"{str(r['nome'])} - Tel: {str(r['telefone'])} - {str(r['cidade']).upper()}/{str(r['est']).upper()}")
            ...
            if prestador_sel == "Outro (Digitar Manualmente)":
                ...
                prestador_final, tel_prestador_final = prestador_limpo.split(" - Tel:")[0], apenas_numeros_letras(prestador_limpo.split(" - Tel:")[1].split("-")[0].strip())
            
            st.markdown("##### 📍 Endereços de Origem e Destino")
            st.info("Caso o cliente não saiba explicar onde está, use o botão verde abaixo para enviar o link de captura de GPS.")
            ...
            with col_loc1: st.markdown(f'<a href="{link_w_loc}" target="_blank"><button style="background-color: #25D366; color: white; padding: 10px; width: 100%; border: none; border-radius: 5px; font-weight: bold; cursor: pointer;">📲 1. Enviar Link no WhatsApp do Cliente</button></a>', unsafe_allow_html=True)
            with col_loc2:
                if st.button("🔄 2. Puxar Localização Recebida do Cliente", use_container_width=True, key="btn_puxar_loc_admin"):
                    ...
                    if not loc_cliente.empty:
                        st.session_state.os_loc_val = loc_cliente.iloc[-1]['link_maps']
            ...
            if c_orig1.button("🔍 Buscar Origem", use_container_width=True, key="btn_cep_orig_admin"):
                ...
                if end_orig: st.session_state.os_loc_val = end_orig; st.rerun()
            ...
            if c_dest1.button("🔍 Buscar Destino", use_container_width=True, key="btn_cep_dest_admin"):
                ...
                if end_dest: st.session_state.os_dest_val = end_dest; st.rerun()
                else: st.error("CEP inválido ou não encontrado.")
            ...
            if st.button("🚀 Finalizar e Gerar OS do Guincho", type="primary", key="btn_gerar_os_admin"):
        ...
        if os_id_edit:
            ...
            if not os_encontrada.empty:
                ...
                with st.form(f"form_edit_os_{os_id_edit}"):
                    ...
                    novo_status = c_os2.selectbox("Status da OS:", status_opcoes, index=status_opcoes.index(row_os['status_os'].upper()) if row_os['status_os'].upper() in status_opcoes else 0)
        ...
        if visao_relatorio == "🚨 OS em Andamento (Gerenciar)":
            ...
            if df_abertas.empty: st.success("Nenhum chamado pendente no momento!")
                ...
                with col_btn2:
                    if st.button("🔒 2. Encerrar OS no Sistema" if status_dessa_os == 'FINALIZADO PELO PRESTADOR' else "🔒 2. Forçar Encerramento da OS Manualmente", key="btn_encerrar_os_adm"):
        ...
        opcao_cli = st.radio("Ação Clientes:", opcoes_radio, horizontal=True, index=opcoes_radio.index(st.session_state.aba_cli) if st.session_state.aba_cli in opcoes_radio else 0)
        ...
        if opcao_cli == "Listar":
            ...
            if df_clientes.empty: st.info("Nenhum cliente cadastrado.")
                ...
                def formatar_historico(c_id):
                    ...
                    for _, r in os_cli.iterrows():
                        try: res.append(f"{r['tipo_servico']} ({datetime.strptime(str(r['data_hora']), '%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y')})")
                        except: res.append(f"{r['tipo_servico']} ({str(r['data_hora'])[:10]})")
                    return " | ".join(res)
                ...
                if len(empresas_na_lista) == 0: st.warning("Nenhum cliente encontrado com esse termo.")
                    ...
                    for emp in empresas_na_lista:
                        ...
                        with st.expander(f"📁 Clientes da Empresa: {nome_emp}", expanded=expandir_pastas):
                            ...
                            cli_sel = st.selectbox(f"🔍 Selecione um cliente da {nome_emp} para ver a Ficha Completa:", cli_opcoes, index=cli_opcoes.index(st.session_state[key_sel_admin]) if st.session_state[key_sel_admin] in cli_opcoes else 0, key=f"sel_sb_{emp}")
                            ...
                            if cli_sel != "":
                                ...
                                c2.write(f"**Endereço:** {cli_data.get('endereco', 'N/D')} - {cli_data.get('cidade', 'N/D')}/{cli_data.get('est', 'N/D')}"); c2.write(f"**Status:** {'🟢 Ativo' if cli_data['status'] == 'Ativo' else '🔴 Inativo'}"); c2.write(f"**Data de Cadastro:** {dt_cad_cliente}")
            ...
            frota_editada = st.data_editor(pd.DataFrame([{"Modelo/Ano": "", "Placa": ""}]), num_rows="dynamic", use_container_width=True)
            st.write("---")
            ...
            if st.button("Salvar Novo Cliente"):
                ...
                if not nome or not pla_prin: st.error("Nome e ao menos 1 Placa de Veículo são obrigatórios.")
                    ...
                    with st.spinner("Salvando novo cliente..."):
                        ...
                        sucesso, erro = salvar_dados(pd.concat([df_clientes, novo], ignore_index=True), FILE_CLIENTES)
                        if sucesso:
                            ...
                            st.session_state.aba_cli = "Listar"; time.sleep(1); st.rerun()
                        else: st.error(f"⚠️ Erro ao salvar cliente na nuvem: {erro}")
            ...
            if arquivo_csv_upload is not None:
                if st.button("Iniciar Importação e Salvar no GitHub"): st.info("Atenção: A função de importação necessita da biblioteca pandas.")
                    ...
                    if st.button("Salvar Alterações"):
                        ...
                        if not nome or not pla_prin: st.error("Nome e ao menos 1 Placa de Veículo são obrigatórios.")
                            ...
                            with st.spinner("Sincronizando edição..."):
                                ...
                                else: st.error(f"⚠️ Erro ao salvar edição na nuvem: {erro}")
        ...
        opcao_emp = st.radio("Ação Empresas:", opcoes_radio_emp, horizontal=True, index=opcoes_radio_emp.index(st.session_state.aba_emp) if st.session_state.aba_emp in opcoes_radio_emp else 0)
        ...
        if opcao_emp == "Listar":
            ...
            if st.button("Salvar Nova Empresa"):
                ...
                if not cnpj or not n_emp_in: st.error("CNPJ e Nome da Empresa são obrigatórios.")
                    ...
                    with st.spinner("Salvando empresa..."):
                        ...
                        sucesso, erro = salvar_dados(pd.concat([df_empresas, novo_e], ignore_index=True), FILE_EMPRESAS)
                        if sucesso:
            ...
            if df_empresas.empty: st.warning("Nenhuma empresa cadastrada.")
                ...
                if e_target != "":
                    ...
                    dia_v_e = c_v3.selectbox("Dia do Vencimento:", OPCOES_DIAS_VENC, index=OPCOES_DIAS_VENC.index(venc_ant) if venc_ant in OPCOES_DIAS_VENC else OPCOES_DIAS_VENC.index("30"))
        ...
        if not pendentes.empty:
            ...
            for idx, p in pendentes.iterrows():
                with st.expander(f"Solicitação de: {p['nome']} - {p['est']}"):
                    ...
                    st.markdown(f'<a href="https://api.whatsapp.com/send?phone=55{apenas_numeros_letras(p['telefone'])}&text={texto_zap}" target="_blank" style="text-decoration: none;"><button style="background-color: #25D366; color: white; padding: 6px 12px; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; margin-bottom: 10px;">📲 Avisar no WhatsApp</button></a>', unsafe_allow_html=True)
                    col_h1, col_h2 = st.columns(2)
                    if col_h1.button("✅ Aprovar no Sistema", key=f"apr_{p['id']}"):
                        df_prestadores.loc[df_prestadores['id'] == p['id'], 'homologado'] = 'Aprovado'
                        if salvar_dados(df_prestadores, FILE_PRESTADORES)[0]: 
        ...
        opcao_pre = st.radio("Ação Prestadores:", opcoes_radio_pre, horizontal=True, index=opcoes_radio_pre.index(st.session_state.aba_pre) if st.session_state.aba_pre in opcoes_radio_pre else 0)
                ...
                if len(estados_na_lista) == 0: st.warning("Nenhum prestador encontrado.")
                    ...
                    for est_sigla in sorted(estados_na_lista):
                        ...
                        with st.expander(f"📁 Prestadores do Estado: {nome_est}", expanded=expandir_pastas_pre):
                            st.dataframe(df_view_pres[df_view_pres['est'] == est_sigla][['nome','cpf','tipo','telefone','cidade','status','homologado']].style.map(colorir_status, subset=['status']), use_container_width=True)
            ...
            if st.button("Salvar Novo Prestador"):
                ...
                if not n_prest_in or not cpf_p: st.error("O Nome e o CPF/CNPJ do prestador são obrigatórios.")
                    ...
                    with st.spinner("Salvando prestador..."):
                        ...
                        sucesso, erro = salvar_dados(pd.concat([df_prestadores, novo_p], ignore_index=True), FILE_PRESTADORES)
                        if sucesso:
        ...
        if df_nps.empty: st.info("Nenhuma avaliação recebida ainda. Os dados aparecerão aqui assim que os clientes começarem a responder.")
        else:
        ...
        if empresas_ativas.empty: st.warning("Nenhuma empresa ativa cadastrada para gerar o financeiro.")
            ...
            for _, emp_row in empresas_ativas.iterrows():
                ...
                if existe.empty:
                    df_financeiro = pd.concat([df_financeiro, pd.DataFrame([{'id': id_unico, 'mes_ano': mes_filtro, 'empresa': nome_emp, 'valor_faturado': '0.00', 'valor_pago': '0.00', 'status': 'Pendente'}])], ignore_index=True)
                    alterado = True
            if alterado:
        ...
        if df_logs.empty: st.info("Nenhuma atividade registrada ainda.")
            ...
            if log_selecionado != "":
                ...
                st.markdown(f"""<div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #7B2CBF; margin-bottom: 15px;"><p style="margin-bottom:5px;"><strong>🕒 Data/Hora:</strong> {detalhe_row['data_hora']}</p><p style="margin-bottom:5px;"><strong>👤 Usuário:</strong> {detalhe_row['usuario']}</p><p style="margin-bottom:5px;"><strong>⚙️ Ação:</strong> {detalhe_row['acao']}</p><p style="margin-bottom:5px;"><strong>📝 Detalhes Completos:</strong> {detalhe_row['detalhes']}</p></div>""", unsafe_allow_html=True)
                if st.button("❌ Excluir este registro selecionado"):
                    with st.spinner("Removendo registro..."):
                        df_logs = df_logs.drop(int(log_selecionado))
                        salvar_dados(df_logs, FILE_LOGS)
            ...
            if st.session_state.confirmar_limpeza_total:
                ...
                if c1.button("✅ Sim, apagar tudo"):
                    ...
                    salvar_dados(df_logs_vazio, FILE_LOGS)
...
# ===================================================================================
# INTERFACE 2: PARCEIROS E INTERFACE 3: PRESTADORES
# ===================================================================================
    ...
    if aba_selecionada != aba_atual_parc: st.query_params["nav"] = aba_selecionada; st.rerun()
    ...
    if aba_selecionada == "📈 Dashboard":
        ...
        dia_v_p0 = "30"
        if not dados_emp_base_p0.empty: dia_v_p0 = str(dados_emp_base_p0.iloc[0].get('dia_vencimento', '30')).strip()
        ...
        op_part = st.radio("Ação Parceiro:", opcoes_radio_part, horizontal=True, index=opcoes_radio_part.index(st.session_state.aba_part) if st.session_state.aba_part in opcoes_radio_part else 0)
        ...
        if op_part == "Visualizar":
            ...
            if df_view_cli_part.empty: st.info("Nenhum cliente encontrado.")
                ...
                cli_sel_part = st.selectbox("🔍 Selecione um cliente para ver a Ficha Completa:", cli_opcoes_part, index=cli_opcoes_part.index(st.session_state.sel_det_part) if st.session_state.sel_det_part in cli_opcoes_part else 0, key="sb_det_part_wid")
                ...
                if cli_sel_part != "":
                    ...
                    if st.button("❌ Fechar Ficha do Cliente", key="btn_close_part"):
                        st.session_state.sel_det_part = ""; st.rerun()
            ...
            if st.button("Salvar Novo Registro"):
                ...
                if not p_nome_in or not pla_prin_p: st.error("Nome e ao menos 1 Placa são obrigatórios.")
                    ...
                    with st.spinner("Salvando novo registro e sincronizando com a nuvem..."):
                        ...
                        else: st.error("⚠️ Atenção: Instabilidade na Conexão com a Nuvem.")
                    ...
                    if st.button("Salvar Alterações"):
                        ...
                        if not p_nome_in or not pla_prin_p: st.error("Nome e ao menos 1 Placa são obrigatórios.")
                            ...
                            with st.spinner("Atualizando cadastro na nuvem..."):
                                ...
                                else: st.error("⚠️ Atenção: Falha de comunicação com a nuvem.")
        ...
        if not dados_emp_base_p.empty: modo_fat_p = str(dados_emp_base_p.iloc[0].get('modo_faturamento', 'Tradicional')).strip()
        ...
        if df_fin_parc.empty: st.info("Nenhum faturamento gerado ou disponível para visualização neste ciclo ainda.")
        else:
            row_fin = df_fin_parc.iloc[0]
            v_fat, v_pag, status_f = str(row_fin.get('valor_faturado', '0.00')), str(row_fin.get('valor_pago', '0.00')), str(row_fin.get('status', 'Pendente')).strip()
            st.markdown("---")
            c_f1, c_f2, c_f3 = st.columns(3)
        ...
        empresa_upper, user_upper = st.session_state.empresa_vinculada.upper(), st.session_state.user.upper()
        ...
        if df_logs_parc.empty: st.info("Nenhuma atividade registrada por sua empresa ou central ainda.")
            ...
            if log_sel_p != "":
                ...
                st.markdown(f"""<div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #7B2CBF; margin-bottom: 15px;"><p style="margin-bottom:5px;"><strong>🕒 Data/Hora:</strong> {detalhe_row['data_hora']}</p><p style="margin-bottom:5px;"><strong>👤 Feito por:</strong> {detalhe_row['usuario']}</p><p style="margin-bottom:5px;"><strong>⚙️ Ação:</strong> {detalhe_row['acao']}</p><p style="margin-bottom:5px;"><strong>📝 Detalhes Completos do Evento:</strong> {detalhe_row['detalhes']}</p></div>""", unsafe_allow_html=True)
            st.write("---")
    ...
    if aba_selecionada != aba_atual_prest: st.query_params["nav"] = aba_selecionada; st.rerun()
    ...
    if aba_selecionada == "🚨 Chamados Ativos":
        ...
        if meus_chamados.empty: st.success("🎉 Nenhuma ordem de serviço pendente para você no momento. Aguarde novos chamados.")
            ...
            for _, os_row in meus_chamados.iterrows():
                ...
                if status_atual_prestador == 'FINALIZADO PELO PRESTADOR': st.success("🏁 Você já chegou ao destino e finalizou esta OS! O veículo foi entregue. Aguardando a Central AD confirmar o encerramento do chamado no sistema.")
                    ...
                    if not st.session_state[key_validada]:
                        ...
                        if c_val1.button("✅ Confirmar Chegada no Local", type="primary", key=f"btn_val_{os_row['id']}"):
                            ...
                            if digitado_limpo == ultimos_3_reais and digitado_limpo != "":
                                ...
                                time.sleep(1.5); st.rerun()
                            else: st.error("❌ Dígitos incorretos. Verifique a placa do veículo físico e tente novamente.")
                    else:
                        ...
                        if passo < 5: 
                            ...
                            if img_capturada:
                                with open(os.path.join(vistoria_path, f"{fotos_necessarias[passo]}.jpg"), "wb") as f_img: f_img.write(img_capturada.getbuffer())
                                st.success(f"✅ Foto salva!")
                                if st.button("Confirmar e Avançar ➡️", key=f"btn_next_{os_row['id']}_{fotos_necessarias[passo]}"):
                                    st.session_state.passo_vistoria += 1; st.rerun()
                            if passo > 0:
                                if st.button("🔄 Reiniciar Fotos", key=f"btn_reset_{os_row['id']}"): st.session_state.passo_vistoria = 0; st.rerun()
                        elif passo == 5: 
                            ...
                            if st.button("🔄 Voltar para a última foto", key=f"btn_voltar_ass"): st.session_state.passo_vistoria = 4; st.rerun()
        ...
        if meu_historico.empty: st.info("Você ainda não possui serviços finalizados no histórico.")
        else:
            ...
            historico_limpo = historico_limpo.rename(columns={'id': 'OS', 'data_hora': 'Data/Hora', 'cliente_nome': 'Cliente', 'placa': 'Placa', 'tipo_servico': 'Serviço', 'destino': 'Destino', 'status_os': 'Status'})
